...
static void add_pinyin_plugin(lv_obj_t * kb){  //给kb添加中文支持, 返回拼音输入法插件对象
    ...
    lv_obj_set_width(cand_panel, lv_obj_get_width(kb)); // 绑定后续按字宽度到键盘
...
lv_obj_t * tools_create_pinyin_ime(lv_obj_t * scr_act, int weight, int height){ //传入放置位置对象
    tools_cn_kb = lv_keyboard_create(scr_act);   //屏幕上添加键盘
